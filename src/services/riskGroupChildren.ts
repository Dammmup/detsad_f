...
export const getRiskGroupChildren = async (
  params?: any,
): Promise<RiskGroupChild[]> => {
  const { data } = await api.get('/risk-group-children', { params });
  return data;
};

export const createRiskGroupChild = async (
  record: Partial<RiskGroupChild>,
): Promise<RiskGroupChild> => {
  const { data } = await api.post('/risk-group-children', record);
  return data;
};

export const updateRiskGroupChild = async (
  id: string,
  record: Partial<RiskGroupChild>,
): Promise<RiskGroupChild> => {
