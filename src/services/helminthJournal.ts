...
export const getHelminthRecords = async (
  params?: any,
): Promise<HelminthRecord[]> => {
  const { data } = await api.get('/helminth-journal', { params });
  return data;
};

export const createHelminthRecord = async (
  record: Partial<HelminthRecord>,
): Promise<HelminthRecord> => {
  const { data } = await api.post('/helminth-journal', record);
  return data;
};

export const updateHelminthRecord = async (
  id: string,
  record: Partial<HelminthRecord>,
): Promise<HelminthRecord> => {
