...
export const getSomaticRecords = async (params?: any): Promise<SomaticRecord[]> => {
  const { data } = await api.get('/somatic-journal', { params });
  return data;
};

export const createSomaticRecord = async (record: Partial<SomaticRecord>): Promise<SomaticRecord> => {
  const { data } = await api.post('/somatic-journal', record);
  return data;
};

export const updateSomaticRecord = async (id: string, record: Partial<SomaticRecord>): Promise<SomaticRecord> => {
