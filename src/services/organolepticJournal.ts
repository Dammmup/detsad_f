...
export const getOrganolepticRecords = async (
  params?: any,
): Promise<OrganolepticRecord[]> => {
  const { data } = await api.get('/organoleptic-journal', { params });
  return data;
};

export const createOrganolepticRecord = async (
  record: Partial<OrganolepticRecord>,
): Promise<OrganolepticRecord> => {
  const { data } = await api.post('/organoleptic-journal', record);
  return data;
};

export const updateOrganolepticRecord = async (
  id: string,
  record: Partial<OrganolepticRecord>,
): Promise<OrganolepticRecord> => {
...
export const generateOrganolepticByMenu = async (params: {
  date?: string;
  group?: string;
}): Promise<OrganolepticRecord[]> => {
  const { data } = await api.post(
    '/organoleptic-journal/generate-by-menu',
    params,
  );
  return data;
};
