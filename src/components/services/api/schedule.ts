 ...
 */
const generateMockShifts = (startDate: string, endDate: string, userId?: string): Shift[] => {
  const shifts: Shift[] = [];
  ...
  // Типы смен
  const shiftTypes = ['regular', 'overtime', 'sick', 'vacation'];
  
  // Статусы смен
  const statuses = ['planned', 'completed', 'cancelled'];
  
  // Генерация смен для каждого дня в диапазоне
  for (let day = new Date(start); day <= end; day.setDate(day.getDate() + 1)) {
    const date = day.toISOString().split('T')[0];
    
    // Если указан userId, генерируем смены только для этого пользователя
    const targetUsers = userId ? users.filter(user => user.id === userId) : users;
    
    targetUsers.forEach(user => {
      // Генерируем случайное количество смен для пользователя в этот день (0-1)
      const shiftsCount = Math.floor(Math.random() * 2);
      
      for (let i = 0; i < shiftsCount; i++) {
        // Случайный тип смены
        const type = shiftTypes[Math.floor(Math.random() * shiftTypes.length)];
        
        // Случайный статус смены
        ...
        // Для сверхурочных смен - вечернее время
        if (type === 'overtime') {
          startTime = '17:00';
          endTime = '21:00';
        }
        ...
        shifts.push({
          ...
          endTime,
          type,
          notes: `${type === 'regular' ? 'Обычная смена' : type === 'overtime' ? 'Сверхурочная работа' : type === 'sick' ? 'Больничный' : 'Отпуск'}`,
          status
        });
      }
    });
