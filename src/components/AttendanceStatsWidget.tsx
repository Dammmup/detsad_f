...
const AttendanceStatsWidget: React.FC<AttendanceStatsWidgetProps> = ({ onStatsChange }) => {
    ...
    const fetchAttendanceStats = async () => {
      ...
      try {
        ...
        const chartDataArray = [
          { name: 'Присутствия', value: statsData.byStatus.present || 0, color: '#4caf50' },
          { name: 'Отсутствия', value: statsData.byStatus.absent || 0, color: '#f44336' },
          { name: 'Опоздания', value: statsData.byStatus.late || 0, color: '#ff9800' },
          { name: 'Болеют', value: statsData.byStatus.sick || 0, color: '#2196f3' },
          { name: 'Отпуск', value: statsData.byStatus.vacation || 0, color: '#9c27b0' }
        ];
  ...
  const getStatusLabel = (status: string) => {
    switch (status) {
      case 'present': return 'Присутствия';
      case 'absent': return 'Отсутствия';
      case 'late': return 'Опоздания';
      case 'sick': return 'Болеют';
                ...
                <Card sx={{ 
                  ...
                  <CardContent>
                    <Typography variant="h6" align="center" color="error.main">
                      {(stats.byStatus.absent || 0) + (stats.byStatus.sick || 0)}
                    </Typography>
                    <Typography variant="body2" align="center" color="text.secondary">
                      Отсутствия
                    </Typography>
