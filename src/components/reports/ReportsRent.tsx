    ...
    const load = async () => {
      ...
      try {
        ...
        setRows(data.map((p: any) => ({
          ...
          paidAmount: Math.max(0, p.paidAmount || (p.accruals && p.total ? (p.accruals - p.total) : 0) || 0), // Вычисляем оплаченную сумму как разницу между начислениями итогом, не допускаем отрицательных значений
  ...
  try {
    ...
    if (originalRow) {
      ...
      const updatedData = {
        ...
        paidAmount: Math.max(0, editData.paidAmount !== undefined ? editData.paidAmount : originalRow.paidAmount),
        // Вычисляем total как сумму аренды минус оплаченная сумма, не допускаем отрицательных значений
        total: Math.max(0, (editData.amount !== undefined ? editData.amount : originalRow.amount) -
...
const handleExportToExcel = () => {
  ...
  // Заголовки
  csvContent += "Арендатор;Сумма аренды;Оплачено/Предоплачено;Остаток к оплате;Статус;Дата оплаты\n";
  ...
  link.setAttribute("download", `арендные_платежи_${new Date().toLocaleString('ru-RU', { month: 'long', year: 'numeric' })}.csv`);
...
const handleGenerateRentSheets = async () => {
  ...
  if (window.confirm(`Вы уверены, что хотите сгенерировать арендные листы за ${monthToGenerate}? Это действие создаст новые листы для ${selectedTenantIds.length > 0 ? 'выбранных арендаторов' : 'всех арендаторов'}.`)) {
    try {
      setGenerating(true);
    ...
    <Box sx={{
      ...
      <Box sx={{
        ...
        {selectedTenantIds.length > 0 && (
          <Box sx={{ mb: 2, p: 2, background: 'linear-gradient(135deg, #e3f2fd 0%, #bbdefb 100%)', borderRadius: 2 }}>
            ...
            <Box sx={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 3 }}>
              ...
              <Box sx={{
                ...
                <Typography variant="h4" sx={{ fontWeight: 'bold', color: 'error.main', mb: 1 }}>
                  {rows.filter(r => isOverduePayment(r.paymentDate)).reduce((sum, r) => {
                    const diff = (r.amount || r.accruals || 0) - r.paidAmount;
                    return sum + Math.max(0, diff); // Не допускаем отрицательных значений
                  }, 0).toLocaleString()} тг
                </Typography>
              ...
              <Table size="medium" sx={{
                ...
                <TableHead>
                  <TableRow sx={{
                    ...
                    <TableCell sx={{ color: 'primary.main', fontWeight: 'bold' }}>Арендатор</TableCell>
                    <TableCell align="right" sx={{ color: 'primary.main', fontWeight: 'bold' }}>Сумма аренды</TableCell>
                    <TableCell align="right" sx={{ color: 'primary.main', fontWeight: 'bold' }}>Оплачено/Предоплачено</TableCell>
                    <TableCell align="right" sx={{ color: 'primary.main', fontWeight: 'bold' }}>Остаток к оплате</TableCell>
                    <TableCell align="right" sx={{ color: 'primary.main', fontWeight: 'bold' }}>Дата оплаты</TableCell>
                    <TableCell align="right" sx={{ color: 'primary.main', fontWeight: 'bold' }}>Статус</TableCell>
                    ...
                    >
                      ...
                      <TableCell align="right" sx={{ fontSize: '1rem', fontWeight: 'medium', color: 'success.main' }}>
                        {editingId === r.tenantId ? (
                          ...
                          r.amount ? r.amount?.toLocaleString() : '0'
                        )}
                      </TableCell>
                      <TableCell align="right" sx={{ fontSize: '1rem', fontWeight: 'medium', color: 'success.main' }}>
                        {editingId === r.tenantId ? (
                          <TextField
                            ...
                            onChange={(e) => handleInputChange('paidAmount', e.target.value ? Number(e.target.value) : '')}
                            inputProps={{ style: { fontSize: 12 } }}
                            InputProps={{ style: { padding: '2px 4px' } }}
                            style={{ width: '80px' }}
                            variant="outlined"
                          ...
                          r.paidAmount ? r.paidAmount?.toLocaleString() : '0'
                        )}
                      </TableCell>
                      <TableCell align="right" sx={{ fontWeight: 'bold', fontSize: '1.1rem', color: ((r.amount || r.accruals || 0) - r.paidAmount) >= 0 ? 'success.main' : 'error.main' }}>
                      ...
                      <TableCell align="right" sx={{ fontSize: '1rem', color: 'text.secondary' }}>
                        {editingId === r.tenantId ? (
                          ...
                          r.paymentDate ? new Date(r.paymentDate).toLocaleDateString('ru-RU') : 'Не оплачено'
                        )}
                      </TableCell>
                      <TableCell>
                        {editingId === r.tenantId ? (
                          ...
                          <Chip
                            label={
                              isOverduePayment(r.paymentDate) ? 'Просрочен' :
                              r.status === 'active' ? 'Активен' : 'Оплачен'
                            }
                            size="medium"
                            color={
                              isOverduePayment(r.paymentDate) ? 'warning' :
                            ...
                            sx={{
                              ...
                              backgroundColor:
                                isOverduePayment(r.paymentDate) ? 'error.light' : // Красный для просроченных
                                r.status === 'paid' ? 'success.light' : // Зеленый для оплаченных
                                'default'
                            }}
                          />
                        )}
