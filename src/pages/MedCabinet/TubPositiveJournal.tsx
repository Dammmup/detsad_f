...
export default function TubPositiveJournal() {
  ...
  const filteredRecords = useMemo(() => {
    ...
    if (doctor)
      filtered = filtered.filter((r) =>
        r.doctor.toLowerCase().includes(doctor.toLowerCase()),
      );
    return filtered;
    ...
    <Box sx={{ p: { xs: 1, md: 3 } }}>
      ...
      <Dialog open={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        <DialogContent>
          ...
          <TextField
            label='Направление'
            value={newRecord.referral || ''}
            onChange={(e) =>
              setNewRecord((r) => ({ ...r, referral: e.target.value }))
            }
            fullWidth
            margin='dense'
          />
          <TextField
            ...
            onChange={(e) =>
              setNewRecord((r) => ({ ...r, doctor: e.target.value }))
            }
            fullWidth
            margin='dense'
          />
