...
import { staffAttendanceService } from '../services/staffAttendance';
import { getChildAttendance } from '../services/childAttendance';
import { getPayrollsByUsers } from '../services/payroll';
    ...
    const loadStatistics = async () => {
        ...
        try {
            ...
            const [users, children, groups, staffAttendanceRes, childAttendance, payrolls] = await Promise.all([
                ...
                childrenApi.getAll(),
                getGroups(),
                staffAttendanceService.getAllRecords({ startDate: startOfMonth, endDate: endOfMonth }).catch(() => ({ data: [] })),
            ...
            const totalSalaryFund = payrolls.reduce((sum: number, p: any) => sum + (p.totalAmount || 0), 0);
            const avgSalary = payrolls.length > 0 ? Math.round(totalSalaryFund / payrolls.length) : 0;
            const totalBonuses = payrolls.reduce((sum: number, p: any) => sum + (p.bonusAmount || 0), 0);
            const totalPenalties = payrolls.reduce((sum: number, p: any) => sum + (p.penaltyAmount || 0), 0);
