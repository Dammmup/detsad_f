...
const Cyclogram: React.FC = () => {
  ...
  const [cyclogramFormErrors, setCyclogramFormErrors] = useState<{
    [key: string]: string;
  }>({});
  ...
  const validateCyclogramForm = () => {
    ...
    if (!cyclogramForm.weekStartDate)
      errors.weekStartDate = 'Выберите дату начала недели';
  ...
  const handleSaveCyclogram = async () => {
    ...
    try {
      if (cyclogramForm.id) {
        ...
        setCyclograms(
          cyclograms.map((c) =>
            c.id === updatedCyclogram.id ? updatedCyclogram : c,
          ),
        );
      } else {
  ...
  return (
    <Paper sx={{ p: 3, m: 2 }}>
                      ...
                      <CardContent>
                        ...
                        <Box mb={2}>
                          <Typography variant='body2'>
                            ...
                            {group?.name || 'Не указана'}
                          </Typography>
                          <Typography variant='body2'>
                            <strong>Воспитатель:</strong>{' '}
                            {teacher?.fullName || 'Не указан'}
                          </Typography>
                          <Typography variant='body2'>
                        ...
                        <Box mb={2}>
                          ...
                          {cyclogram.timeSlots.length > 3 && (
                            <Typography variant='body2' color='text.secondary'>
                              ... и еще {cyclogram.timeSlots.length - 3}{' '}
                              активностей
                            </Typography>
                          )}
                        </Box>
                        ...
                        <Box display='flex' justifyContent='flex-end' gap={1}>
                          ...
                          <Tooltip title='Удалить'>
                            <IconButton
                              size='small'
                              onClick={() =>
                                handleDeleteCyclogram(cyclogram.id || '')
                              }
                            >
                  ...
                  <Card>
                    <CardContent>
                      ...
                      <Typography variant='body2' gutterBottom>
                        <strong>Возрастная группа:</strong> {template.ageGroup}{' '}
                        лет
                      </Typography>

                      <Typography variant='body2' gutterBottom>
                        <strong>Активностей:</strong>{' '}
      ...
      <Dialog
        open={cyclogramDialogOpen}
        onClose={() => setCyclogramDialogOpen(false)}
        maxWidth='md'
        fullWidth
      >
        <DialogTitle>
          {selectedCyclogram
            ? 'Редактировать циклограмму'
