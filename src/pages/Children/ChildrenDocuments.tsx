...
interface ChildDocument {
  id: string;
  title: string;
 type: 'medical' | 'educational' | 'certificate' | 'other';
  childId: string;
  childName: string;
  fileName: string;
  fileSize: number;
...
const ChildrenDocuments: React.FC = () => {
  ...
  const [currentDocument, setCurrentDocument] = useState<Partial<ChildDocument> | null>(null);
    ...
    const loadData = async () => {
      ...
      try {
        ...
        const childrenDocuments = documentsData.data?.map((doc: any) => ({
          ...
          title: doc.title,
          type: doc.type,
          childId: doc.relatedId,
          childName: doc.uploader?.fullName || 'Неизвестный',
        ...
        setError('Не удалось загрузить документы детей. Пожалуйста, обновите страницу.');
  ...
  useEffect(() => {
    ...
    // Фильтр по ребенку
    if (filterChild) {
      filtered = filtered.filter(doc => doc.childId === filterChild);
    }
  ...
  }, [documents, searchTerm, filterType, filterChild, filterStatus]);
  ...
  const handleOpenDialog = (document?: ChildDocument) => {
  ...
  const handleSaveDocument = async () => {
    ...
    try {
      // Преобразование типа документа для API
      const mapDocumentTypeToAPI = (type: string): 'certificate' | 'other' | 'contract' | 'report' | 'policy' => {
        switch (type) {
          case 'medical':
            return 'certificate';
          case 'educational':
      ...
      const documentData = {
        ...
        type: mapDocumentTypeToAPI(currentDocument.type || 'other'),
        category: 'children' as const,
        relatedId: currentDocument.childId,
        relatedType: 'child' as const,
      ...
      if (currentDocument.id) {
        ...
        setDocuments(documents.map(doc =>
          doc.id === currentDocument.id ? {
            ...
            // Преобразуем тип обратно к типу компонента
            type: doc.type,
            childId: savedDocument.relatedId,
            childName: savedDocument.uploader?.fullName || doc.childName,
        ...
        // Находим имя ребенка по ID
        const child = children.find(c => c.id === currentDocument.childId);
        
        // Добавляем новый документ в список
        const newDocument: ChildDocument = {
          id: savedDocument.id,
          title: savedDocument.title,
          // Сохраняем оригинальный тип документа
          type: currentDocument.type || 'other',
          childId: savedDocument.relatedId || '',
          childName: child?.fullName || savedDocument.uploader?.fullName || '',
  ...
  const handleDeleteDocument = async (id: string) => {
    if (window.confirm('Удалить документ ребенка?')) {
      try {
  ...
  const handleDownloadDocument = (document: ChildDocument) => {
        ...
        <Box display="flex" flexWrap="wrap" gap={2} alignItems="center">
          ...
          <FormControl size="small" sx={{ minWidth: 150 }}>
            ...
            >
              ...
              <MenuItem value="medical">Медицинский</MenuItem>
              <MenuItem value="educational">Образовательный</MenuItem>
          ...
          <FormControl size="small" sx={{ minWidth: 200 }}>
            <Autocomplete
              ...
              value={children.find(c => c.id === filterChild) || null}
              onChange={(event, newValue) => setFilterChild(newValue?.id || '')}
              renderInput={(params) => (
                <TextField 
                  {...params} 
                  label="Ребенок" 
            ...
            <Grid container spacing={2}>
              ...
              <Grid item xs={12} md={6}>
                <Autocomplete
                  options={children}
                  getOptionLabel={(option) => option.fullName}
                  value={children.find(c => c.id === currentDocument.childId) || null}
                  onChange={(event, newValue) => setCurrentDocument({
                    ...currentDocument,
                    childId: newValue?.id || '',
                    childName: newValue?.fullName || ''
                  })}
                  renderInput={(params) => (
                    <TextField 
                      {...params} 
