...
```
{
  "rewrites": [
    {
      "source": "/((?!static|favicon|manifest|api|_next/static|app/static|assets|images|css|js|png|jpg|jpeg|gif|ico|svg|woff|woff2|ttf|eot).*)",
      "destination": "/"
    }
  ],
  "headers": [
...
Эта конфигурация перенаправляет все маршруты на `/`, за исключением статических файлов (CSS, JS, изображения, шрифты), favicon, manifest файлов, а также API маршрутов и других медиафайлов, что позволяет корректно работать клиентской маршрутизации без ошибок загрузки статических ресурсов.
